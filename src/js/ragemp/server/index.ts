...
export function registerAsyncProcedure(name: string, method: AsyncServerAction): void {
  registry.registerAsyncProcedure(
    name,
    (args: ServerArgs) => method(args.Player, args.Args)
  );
}
...
export function callClientAsync<TArgs>(player: PlayerMp, name: string, args?: TArgs): void {
  controller.callAsync<TArgs | undefined>(
    name,
    (request) => player.call(Event.Noreply, [JSON.stringify(request)]),
    args
  );
}
...
export function callBrowserAsync<TArgs>(player: PlayerMp, name: string, browserId: number, args?: TArgs): void {
  controller.callAsync<TArgs | undefined>(name, (request) => {
...
export function callClientSync<TResult, TArgs = undefined>(
  ...
  return controller.callSync<TArgs | undefined, TResult>(name, timeout, Source.Server, (request) =>
    player.call(Event.Client.ReplyToServer, [JSON.stringify(request)]), args);
}
